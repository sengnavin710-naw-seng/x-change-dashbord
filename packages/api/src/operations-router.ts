    ...
    .query(async ({ ctx, input }) => {
      ...
      const items = [
        ...exchangeRecords.map((record) => ({
          ...
          id: record.id,
          inAmount: record.sourceAmount,
          inChannel: null,
          inCurrency: record.direction === "thb-to-mmk" ? ("THB" as const) : ("MMK" as const),
          outAmount: record.actualPayout,
          outChannel: null,
          outCurrency: record.direction === "thb-to-mmk" ? ("MMK" as const) : ("THB" as const),
        ...
        ...cashBankRecords.map((record) => ({
          ...
          direction: record.direction,
          id: record.id,
          inAmount: record.direction === "bank-to-cash" ? record.bankIn : record.cashIn,
          inChannel: record.direction === "bank-to-cash" ? ("Bank" as const) : ("Cash" as const),
          inCurrency: record.currency,
          outAmount: record.direction === "bank-to-cash" ? record.cashOut : record.bankOut,
          outChannel: record.direction === "bank-to-cash" ? ("Cash" as const) : ("Bank" as const),
